...
WHERE reviews.critic_score IS NULL 
...
-- Group by release year
GROUP BY year
-- filter years with more than four game released
...
SELECT 
  top_critic_years.year,
  top_critic_years.avg_critic_score 
FROM top_critic_years 
LEFT JOIN top_critic_years_more_than_four_games 
ON top_critic_years.year = top_critic_years_more_than_four_games.year
WHERE top_critic_years_more_than_four_games.avg_critic_score IS NULL
ORDER BY top_critic_years.avg_critic_score DESC;
```
...
SELECT 
  top_user_years_more_than_four_games.year
FROM 
